...
function Setting() {
    ...
    if (id) {
      axios.get(`http://10.16.16.108:7000/api/editbook/${id}`)
        .then((response) => {
          ...
          dispatch(updateCreateNewBookField({ field: 'timePeriod', value: response.data.timePeriod }));
          dispatch(updateCreateNewBookField({ field: 'location', value: response.data.location }));
          dispatch(updateCreateNewBookField({ field: 'additionalDetails', value: response.data.additionalDetails }));
        })
        .catch((error) => {
          console.error(error);
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Settings data submitted successfully:', response.data);
                  ...
                  >
                    <Col md="6">
                      ...
                      <Form.Control
                        ...
                        name="timePeriod"
                        value={id ? settingData.timePeriod : formData.timePeriod}
                        onChange={handleChange}
                        isInvalid={!!validationErrors.timePeriod}
                      />
                      <Form.Control.Feedback type="invalid"> {validationErrors.timePeriod}</Form.Control.Feedback>
                    </Col>
                    <Col md="6">
                  ...
                  >
                    <Col md="12">
                      ...
                      <Form.Control
                        ...
                        value={id ? settingData.additionalDetails : formData.additionalDetails}
                        onChange={handleChange}
                        rows={3}
                        isInvalid={!!validationErrors.additionalDetails}
                      />
                      <Form.Control.Feedback type="invalid"> {validationErrors.additionalDetails}</Form.Control.Feedback>
                    </Col>
                  </Row>
