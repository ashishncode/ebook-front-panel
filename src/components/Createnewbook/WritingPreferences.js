...
function WritingPreferences() {
    ...
    if (id) {
      axios.get(`http://10.16.16.108:7000/api/editbook/${id}`)
        .then((response) => {
          ...
          dispatch(updateCreateNewBookField({ field: 'writingStyle', value: response.data.writingStyle }));
          dispatch(updateCreateNewBookField({ field: 'additionalInstruction', value: response.data.additionalInstruction }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://10.16.16.108:7000/api/createbooks', {
        writingStyle,
        additionalInstruction,
      });
      console.log('Writing Preferences submitted successfully:', response.data);
    } catch (error) {
                  ...
                  >
                    <Col md="12">
                      ...
                      <Form.Select aria-label="Children"
                        // value={writingStyle}
                        name="writingStyle"
                        value={id ? writingData.writingStyle : formData.writingStyle}
                      ...
                      <Form.Control.Feedback type="invalid"> {validationErrors.writingStyle}</Form.Control.Feedback>
                    </Col>
                  </Row>
                  <Row
                  ...
                  >
                    <Col md="12">
                      ...
                      <Form.Control
                        ...
                        value={id ? writingData.additionalInstruction : formData.additionalInstruction}
                        onChange={handleChange}
                        required
                        isInvalid={!!validationErrors.additionalInstruction}
                      />
                      <Form.Control.Feedback type="invalid"> {validationErrors.additionalInstruction}</Form.Control.Feedback>
                    </Col>
                  </Row>
